"use client";

import { useEffect, useState } from "react";

export default function ScrollToTop() {
  const [isVisible, setIsVisible] = useState(false);

  // Top: 0 takes us all the way back to the top of the page
  // Behavior: smooth keeps it smooth!
  const scrollToTop = () => {
    window.scrollTo({
      top: 0,
      behavior: "smooth",
    });
  };

  useEffect(() => {
    // Button is displayed after scrolling for 500 pixels
    const toggleVisibility = () => {
      if (window.pageYOffset > 300) {
        setIsVisible(true);
      } else {
        setIsVisible(false);
      }
    };

    window.addEventListener("scroll", toggleVisibility);

    return () => window.removeEventListener("scroll", toggleVisibility);
  }, []);

  return (
    <div className="fixed right-8 bottom-8 z-[99]">
      {isVisible && (
        // biome-ignore lint/a11y/useKeyWithClickEvents: keep
        // biome-ignore lint/a11y/noStaticElementInteractions: keep
        // biome-ignore lint/a11y/useAriaPropsSupportedByRole: keep
        <div
          onClick={scrollToTop}
          aria-label="scroll to top"
          className="flex h-10 w-10 cursor-pointer items-center justify-center rounded-md bg-brand text-white shadow-md transition duration-300 ease-in-out hover:bg-brand/80 hover:shadow-signUp"
        >
          <span className="mt-1.5 h-3 w-3 rotate-45 border-white border-t border-l"></span>
        </div>
      )}
    </div>
  );
}
